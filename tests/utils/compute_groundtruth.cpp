...
inline void load_bin_as_float(const char *filename, float *&data, size_t &npts,
  ...
  // read in batch.
  uint64_t npoint_batch = 10000000;
  for (uint64_t st = 0; st < nptsuint64_t; st += npoint_batch) {
    T *data_T = new T[npoint_batch * ndimsuint64_t];
    uint64_t n_read = npoint_batch;
    if (st + n_read > nptsuint64_t) {
      n_read = nptsuint64_t - st;
    }
    reader.read((char *) data_T, sizeof(T) * n_read * ndimsuint64_t);
    std::cout << "Read part of the bin file: (" << st + n_read << "/" << nptsuint64_t << ")" << std::endl;
#pragma omp parallel for schedule(dynamic, 32768)
    for (int64_t i = 0; i < (int64_t) n_read; i++) {
      for (int64_t j = 0; j < (int64_t) ndimsuint64_t; j++) {
        float cur_val_float = (float) data_T[i * ndimsuint64_t + j];
        std::memcpy((char *) (data + (st + i) * ndimsuint64_t + j),
