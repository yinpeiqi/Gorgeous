...
int main(int argc, char** argv) {
  std::string data_type, dist_fn, data_path, index_path_prefix, pq_path_prefix;
  unsigned    num_threads, R, L, disk_PQ;
  float       B, M;
  bool        append_reorder_data = false;
  int        build_pq_only_int = 0;
  ...
  try {
    ...
    desc.add_options()("pq_path_prefix",
                       ...
                       "Path prefix for saving PQ file components");
    ...
    desc.add_options()("build_pq_only",
                       po::value<int>(&build_pq_only_int)->default_value(0),
                       "only build pq");
    desc.add_options()("n_PQ_code",
                       po::value<uint32_t>(&n_PQ_code)->default_value(0),
                       "number of PQ code");
    desc.add_options()("sector_len",
                       po::value<uint32_t>(&sector_len)->default_value(4096),
                       "sector_len");
  ...
  std::string params = std::string(std::to_string(R)) + " " +
                       ...
                       std::string(std::to_string(build_pq_only)) + " " +
                       std::string(std::to_string(n_PQ_code)) + " " +
  ...
  try {
    if (data_type == std::string("int8"))
      return diskann::build_disk_index<int8_t>(
          data_path.c_str(), index_path_prefix.c_str(), pq_path_prefix.c_str(), params.c_str(), metric, sector_len);
